...
func get_datastore_path() string {
	...
	if err != nil {
		log.Log("Error getting persistence config: "+err.Error(), "ERROR")
	}
...
func Save(datastore *datastore.DataStore) {
	...
	if err == nil {
		...
		if err != nil {
			log.Log("Error creating file: "+err.Error(), "ERROR")
			return
		}
		defer file.Close()
		...
		log.Log("Error checking file existence: "+err.Error(), "ERROR")
		return
	}
}
...
func writeInDataStoreFile(datastore *datastore.DataStore, filepath string) error {
	...
	if err != nil {
		log.Log("Error opening file for writing: "+err.Error(), "ERROR")
		return err
	}
	defer file.Close()
	...
	if err != nil {
		log.Log("Error marshaling datastore to JSON: "+err.Error(), "ERROR")
		return err
	}
	...
	if err != nil {
		log.Log("Error compressing data: "+err.Error(), "ERROR")
		return err
	}

	err = gzipWriter.Close()
	if err != nil {
		log.Log("Error closing gzip writer: "+err.Error(), "ERROR")
		return err
	}
	...
	if err != nil {
		log.Log("Error writing datastore to file: "+err.Error(), "ERROR")
		return err
	}
...
func readDataStoreFromFile(filepath string) (*datastore.DataStore, error) {
	...
	if err != nil {
		log.Log("Error decoding base64 data: "+err.Error(), "ERROR")
		return nil, err
	}
	...
	if err != nil {
		log.Log("Error creating gzip reader: "+err.Error(), "ERROR")
		return nil, err
	}
	...
	if err != nil {
		log.Log("Error decompressing data: "+err.Error(), "ERROR")
		return nil, err
	}
...
func Load() (*datastore.DataStore, error) {
	...
	if os.IsNotExist(err) {
		log.Log("DataStore not found: "+err.Error(), "ERROR")
		return datastore.NewDataStore(), nil
	}
	if err == nil {
		savedDataStore, err := readDataStoreFromFile(datastorepath)
		if err != nil {
			log.Log("Error reading datastore file: "+err.Error(), "ERROR")
